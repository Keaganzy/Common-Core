...
{
	while (--count >= 0)
		pthread_mutex_destroy(&god->forks[count]);
	pthread_mutex_destroy(&god->write_lock);
	pthread_mutex_destroy(&god->spag_lock);
...
void	print_action(t_philo *philo, char *action)
{
	...
	time = get_current_time() - philo->times.from_born;
