...
/*   Updated: 2025/08/03 21:29:52 by ksng             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_tile(t_map *map, int tilex, int tiley)
{
	...
	if (state == '0' || state == 'P' || state == 'C' || state == 'E')
		mlx_put_image_to_window(map->mlx_ptr, map->win_ptr,
			map->img.floor, tilex, tiley);
	if (state == 'P')
		mlx_put_image_to_window(map->mlx_ptr, map->win_ptr,
			map->img.player, tilex, tiley);
	else if (state == 'E')
		mlx_put_image_to_window(map->mlx_ptr, map->win_ptr,
			map->img.exit, tilex, tiley);
	else if (state == 'C')
		mlx_put_image_to_window(map->mlx_ptr, map->win_ptr,
			map->img.collectible, tilex, tiley);
	else if (state == '1')
		mlx_put_image_to_window(map->mlx_ptr, map->win_ptr,
			map->img.wall, tilex, tiley);
}
