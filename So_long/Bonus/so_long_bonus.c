...
/*   Updated: 2025/08/03 23:15:56 by ksng             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long_bonus.h"

void	init_map_m(t_map *map)
{
	map->monster.num = 0;
	map->monster.arr = 0;
...
int	main(int ac, char **av)
{
	...
	if (ac == 2)
	{
		...
		map_print(&map);
		find_monster(&map);
		if (map.monster.num > 0 && map.exit != 1)
			mlx_loop_hook(map.mlx_ptr, move_monster, &map);
